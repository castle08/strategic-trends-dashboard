# Feed Performance Database Setup
...
GROUP BY fc.category_name, fc.target_feed_count, fc.min_success_rate, fc.min_trend_contribution, fc.priority
...
-- Function: Get feeds needing discovery (FIXED)
...
BEGIN
    ...
    GROUP BY fc.category_name, fc.target_feed_count, fc.min_success_rate, fc.priority
...
3. **Copy the entire SQL block above (from `-- Feed Performance Tracking Schema` to the end)**
4. **Paste it into the SQL Editor**
5. **Click "Run" to execute**

## What This Creates:

- **`feed_performance`** table - Tracks RSS feed performance metrics
- **`feed_discovery_history`** table - Records discovered and tested feeds  
- **`feed_categories`** table - Defines categories and performance targets
- **Performance views** - Easy access to feed and category performance data
- **Helper functions** - For updating performance and finding feeds needing discovery
...
SELECT * FROM get_feeds_needing_discovery();
```
